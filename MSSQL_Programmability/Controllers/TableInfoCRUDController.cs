...
{
    /// <summary>
    /// TableInfoViewTableInfo CRUD
    /// </summary>
    public class TableInfoCRUDController : ApiController
    {
        ...
        public IHttpActionResult GetList()
        {
            ...
            List<TableInfoViewTableInfo> fun_and_sps = new List<TableInfoViewTableInfo>();
            try
            {
                using (SQL_Programmability db = new SQL_Programmability())
                {
                    fun_and_sps = db.TableInfoViewTableInfo.OrderByDescending(r => r.CreatedTime).ToList();
                }
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns>回傳所有資料</returns>
        [Route("api/TableInfoCRUD/search")]
        public IHttpActionResult GetListSearch()
        {
            ...
            try
            {
                using (SQL_Programmability db = new SQL_Programmability())
                {
                    ...
                    string sql = "select * from [TableInfoViewTableInfo]";
                    sql += parameterString(parameters);

                    var IQ_fun_and_sps = db.Database.SqlQuery<TableInfoViewTableInfo>(sql, parameters.ToArray());
        ...
        public IHttpActionResult Delete(List<int> ids)
        {
            ...
            List<TableInfoViewTableInfo> proSPs = new List<TableInfoViewTableInfo>();
            try
            {

                using (SQL_Programmability db = new SQL_Programmability())
                {
                    proSPs = db.TableInfoViewTableInfo.Where(r => ids.Any(_id => _id.Equals(r.Id))).ToList();
